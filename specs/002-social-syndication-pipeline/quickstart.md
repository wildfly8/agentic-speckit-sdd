...
- An X Developer App, a Facebook Business App + Page, and (optionally, best-effort) a Medium
  integration token — see full credential setup in
...
2. Add repo **secrets**: `X_APP_KEY`, `X_APP_SECRET`, `X_ACCESS_TOKEN`, `X_ACCESS_SECRET`, `FB_PAGE_ID`, `FB_PAGE_ACCESS_TOKEN`, and optionally `MEDIUM_INTEGRATION_TOKEN`.
...
- Triggers `.github/workflows/social-syndicate.yml`, which:
  ...
  - Publishes the full post to Medium (if `MEDIUM_INTEGRATION_TOKEN` is configured — best-effort, since Medium's API is deprecated)
...
- Check your X account, Facebook Page, and Medium profile for the new posts.
